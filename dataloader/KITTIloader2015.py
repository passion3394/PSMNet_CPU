...
def dataloader(filepath):
  ...
  train = image[:int(image_num*0.875)]
  val   = image[int(image_num*0.875):]

  left_train  = [filepath+left_fold+img for img in train]
  right_train = [filepath+right_fold+img for img in train]
  disp_train_L = [filepath+disp_L+img[:-4]+'.png' for img in train]
  ...
  disp_val_L = [filepath+disp_L+img[:-4]+'.png' for img in val]
